...
export default function ParentDetail({onSubmit, onBack, data, setData}) {
    const checkValid = ()=>{
        if(data.father_name?.trim().length >= 3);
        else return "Enter valid father's name";
        if(data.mother_name?.trim().length >= 3);
        else return "Enter valid mother's name";
        ...
        <div className='tw-h-full tw-flex tw-justify-center tw-flex-col tw-items-center'>
            <div className='tw-w-full'>
                <div className='tw-flex tw-flex-col tw-items-center tw-w-full'>
                    <input type="text" name="father_name" placeholder='Father Name*' value={data.father_name} onChange={(e)=>setData({...data, [e.target.name] : e.target.value})} className='tw-border-gray-300 tw-border-2 tw-px-4 tw-py-2 focus:tw-outline-none tw-rounded-sm tw-w-4/5 tw-my-2'/>
                    <input type="text" name="mother_name" placeholder='Mother Name*' value={data.mother_name} onChange={(e)=>setData({...data, [e.target.name] : e.target.value})} className='tw-border-gray-300 tw-border-2 tw-px-4 tw-py-2 focus:tw-outline-none tw-rounded-sm tw-w-4/5 tw-my-2'/>
